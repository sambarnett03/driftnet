...
def main():
    ...
    if args.mean:
        ...
        t0, t1 = ds.time_counter.values[[0, -1]]
        title = f"Mean surface current speed, {str(t0)[:10]} to {str(t1)[:10]}"
    else:
        ...
        title = f"Surface current speed, {str(snap.time_counter.values)[:16].replace('T', ' ')}"
